...
/*
参考博客:
http://www.tuicool.com/articles/bYFzyeI
 */
public class ThreadPoolExecutor extends AbstractExecutorService {
	 ...
	 * mainLock.
	 */
	private int largestPoolSize; //用于状态监控,  最大任务个数
	...
	private final class Worker
		 ...
		 */
		/*
		addWorker()中t.start() 后执行, t就是this
		 */
		public void run() {
			runWorker(this);
		}
    ...
    核心代码: 具体的执行任务
     ...
     3 启动任务,线程数+1
     */
	private boolean addWorker(Runnable firstTask, boolean core) {  //core为true, 则说明当前执行的任务小于corePoolSize;  core为false, 即corePoolSize已经满了, 队列也满了, 需要启动小于maximumPoolSize的线程去执行
		retry:
		for (; ; ) {
			int c = ctl.get();
			int rs = runStateOf(c);
			...
			// 走到这的情形：
			// 1.线程池状态为RUNNING
			// 或 2.SHUTDOWN状态，但队列中还有任务需要执行
			for (; ; ) {
				int wc = workerCountOf(c);
				if (wc >= CAPACITY ||
						    wc >= (core ? corePoolSize : maximumPoolSize)) //线程数量不足
					return false;
		...
		try {
			...
			if (t != null) {
				final ReentrantLock mainLock = this.mainLock; //获得全局锁
				mainLock.lock();
				try {
...
//	线程退出会执行这个方法做一些清理工作
	...
	final void runWorker(Worker w) {
		...
		try {
			...
			while (task != null || (task = getTask()) != null) {//如果task为空,则从队列中获取任务
				w.lock();
	...
	/*
	线程池的具体实现:
	初始值,最大值,保留最大的时间, 这些线程存储在什么样的容器中.
	1 初始值和最大值的设定, 容器的类型 就会有不同的线程池类型
	2 注意 队列中存储的都是Runnable类型,说明异步都没有返回值. 那么如果有返回值呢
	 */
	...
	public ThreadPoolExecutor(int corePoolSize,
	                          ...
	                          long keepAliveTime,
	                          TimeUnit unit,
	                          BlockingQueue<Runnable> workQueue,//必须是阻塞队列;  具体类型是 Runnable , 而不是Callable
	                          ThreadFactory threadFactory,
	                          RejectedExecutionHandler handler) {
		if (corePoolSize < 0 ||
				    maximumPoolSize <= 0 ||
	...
	public void execute(Runnable command) {
		...
		/*
        1 如果 任务数 少于 corePoolSize, 启动一个新的线程去执行task
		...
		/*
		如果 活动线程数 >= corePoolSize
		1 首先把任务放入队列中 ( 不同的线程池队列是不同的,SynchronousQueue 会被阻塞)
		 a  如果 队列没有满,放入成功
		    再一次检查运行状态, 如果 非正常运行, 则在队列中删除任务,并拒绝任务

		 */
		// runState为RUNNING && 队列未满(非阻塞)
		if (isRunning(c) && workQueue.offer(command)) {//100000000000000000000000000000
			int recheck = ctl.get();
			// double check  检查线程池的状态: 如果不再运行,则拒绝任务; 如果是关闭,...
			// 非RUNNING状态 则从workQueue中移除任务并拒绝
			if (!isRunning(recheck) && remove(command))
				reject(command);// 采用线程池指定的策略拒绝任务
			else if (workerCountOf(recheck) == 0)
				// 这行代码是为了SHUTDOWN状态下没有活动线程了，但是队列里还有任务没执行这种特殊情况。
				addWorker(null, false);// 添加一个null任务是因为SHUTDOWN状态下，线程池不再接受新任务
		}
		// 两种情况：
		// 1.非RUNNING状态拒绝新的任务
		// 2.队列满了启动新的线程失败（workCount > maximumPoolSize）
		else if (!addWorker(command, false))//对应: corePoolSize的线程数已经启动了; 队列满了,放入失败; 启动新的线程去执行任务
			reject(command);
	}
